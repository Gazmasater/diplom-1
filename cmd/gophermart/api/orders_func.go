...
func (mc *App) SaveOrderNumber(orderNumber, userEmail string) error {
	log := logger.GetLogger()

	// Проверка на уникальность номера заказа перед вставкой
	var count int
	err := mc.DB.QueryRow("SELECT COUNT(*) FROM orders WHERE order_number = $1", orderNumber).Scan(&count)
	if err != nil {
	...
	_, err = mc.DB.Exec(query, orderNumber, userEmail, createdAt)
	if err != nil {
